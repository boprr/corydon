...
static inline int getexp(unsigned int size) {
    if (size < (1 << MINEXP)) {
#ifdef DEBUG
        printf("[liballoc] getexp returns -1 for %i less than MINEXP\n", size);
#endif
...
#ifdef DEBUG
    printf("[liballoc] getexp returns %i (%i bytes) for %i size\n", shift - 1, (1 << (shift - 1)),
           size);
#endif
...
static void dump_array() {
    ...
    printf("[liballoc] ------ Free pages array ---------\n");
    printf("[liballoc] System memory allocated: %i\n", l_allocated);
    printf("[liballoc] Memory in used (malloc'ed): %i\n", l_inuse);

    for (i = 0; i < MAXEXP; i++) {
        printf("[liballoc] %.2i(%i): ", i, l_completePages[i]);
    ...
    printf("[liballoc] '*' denotes a split to the left/right of a tag\n");
}
#endif
...
#ifdef DEBUG
    printf(
        "[liballoc] Resource allocated %x of %i pages (%i bytes) for %i "
        "size.\n",
...
void *malloc(size_t size) {
...
#ifdef DEBUG
            printf("[liballoc] Tag search found %i >= %i\n",
    ...
    if (((int)(remainder) > 0) /*&& ( (tag->real_size - remainder) >= (1<<MINEXP))*/) {
        ...
        if (childIndex >= 0) {
#ifdef DEBUG
            printf("[liballoc] Seems to be splittable: %i >= 2^%i .. %i\n", remainder, childIndex,
                   (1 << childIndex));
#endif
...
#ifdef DEBUG
            printf(
                "Old tag has become %i bytes, new tag is now %i bytes (%i "
                "exp)\n",
...
#ifdef DEBUG
            l_allocated -= pages * l_pageSize;
            printf("[liballoc] Resource freeing %x of %i pages\n", tag, pages);
            dump_array();
#endif
