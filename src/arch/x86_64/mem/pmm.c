...
void parse_mb2_mmap() {
    ...
    for (mmap = ((struct multiboot_tag_mmap *)mmap_tag)->entries;
        ...
        if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE &&
            (uint64_t)mmap->len >= phys->size / BLOCK_SIZE && !found_map_start) {
        ...
        printf("base: 0x%p | length: % 17ubytes | type: %i\n", mmap->addr, mmap->len, mmap->type);
    }
...
void memset(void *_dst, int val, size_t len) {
    asm volatile("pushf; cld; rep stosb; popf" : : "D"(_dst), "a"(val), "c"(len) : "memory");
}
...
void pmm_init() {
    ...
    printf("[PMM]: BITMAP INFO (%p) - start: %p - size: %ubytes", phys->map, phys->mem_start,
           phys->size);
    ...
    for (mmap = ((struct multiboot_tag_mmap *)mmap_tag)->entries;
         ...
         mmap = (multiboot_memory_map_t *)((unsigned long)mmap + mmap_tag->entry_size)) {
        if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE)
            bitmap_mark_region(phys, (void *)mmap->addr, mmap->len / BLOCK_SIZE, 0);
    }
    bitmap_mark_region(phys, (void *)phys->mem_start, phys->size / BLOCK_SIZE, 1);
